...
bool FindSpecSymb(const string& word) {
    for(const auto& symb : word) {
        if( static_cast<u_int>(symb) <= 31u)
            return true;
    }
    return false;
}
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status,
        ...
        for (const string& word : words) {
            if(FindSpecSymb(word))
                throw invalid_argument("Наличие специальных символов в тексте добавляемого документа"s);
...
int main() {
    try {
        ...
        int doc_id = search_server.GetDocumentId(0);
        cout << "doc_id = "s << doc_id << endl;
